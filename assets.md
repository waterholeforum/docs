...
Add stylesheets to the bundle using the [`Stylesheet`](reference://Waterhole/Extend/Assets/Stylesheet.html) extender. Waterhole will concatenate stylesheets onto the end of the bundle:

```php
use Waterhole\Extend;

$this->extend(function (Extend\Assets\Stylesheet $stylesheets) {
    $stylesheets->add(resource_path('css/styles.css'));
});
```

### Scripts

Add scripts to the bundle using the [`Script`](reference://Waterhole/Extend/Assets/Script.html) extender. Waterhole will concatenate scripts onto the end of the bundle:

```php
use Waterhole\Extend;

$this->extend(function (Extend\Assets\Script $scripts) {
    $scripts->add(resource_path('js/script.js'));
});
```
...
To link to an external asset (e.g. Google Fonts or something else from a CDN), don't use the `Stylesheet` or `Script` extenders. Instead, use the [`DocumentHead` extender](reference://Waterhole/Extend/Ui/DocumentHead.html) to add a view to output in the `<head>` tag.
