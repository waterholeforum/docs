...
The `.btn` class can be applied to any element to give it the appearance of a
button:
...
For toggle buttons, use the `[aria-pressed="true"]` attribute to indicate that
...
`.btn--outline` and `.btn--transparent` are available to create less visually
prominent buttons:
...
Use the `.btn--icon` class to create an icon-only button. Don't forget to add an
`aria-label` attribute or a [tooltip](./tooltips.md).
...
`.btn--sm`, `.btn--wide` and `.btn--narrow` are available to tweak the size of a
button:
